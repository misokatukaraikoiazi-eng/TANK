...
static const int8_t STICK_DEADZONE = 30;
...
static void update_motor_control(bool connected, const ps4_state_t *state) {
    ...
    // Button control has priority for quick directional tests.
    if (state->cross) {
        left_cmd = 1;
        right_cmd = 1;
        ...
        left_cmd = 1;
        right_cmd = -1;
    } else if (state->ly > STICK_DEADZONE) {
        left_cmd = 1;
        right_cmd = 1;
    } else if (state->ly < -STICK_DEADZONE) {
        left_cmd = -1;
        right_cmd = -1;
    } else if (state->lx > STICK_DEADZONE) {
        left_cmd = 1;
        right_cmd = -1;
    } else if (state->lx < -STICK_DEADZONE) {
        left_cmd = -1;
...
void app_main(void) {
    ...
    while (1) {
        ...
        // LEDの制御は20ms周期で爆速処理
        led_update(&state);
        update_motor_control(concheck, &state);
